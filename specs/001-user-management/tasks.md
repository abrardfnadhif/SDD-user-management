...
- [ ] T001 Create project structure per plan (backend/, frontend/, specs/) at `/Users/abrar.d.f.nadhif/project/SDD-FS/` (devops)
...
- [ ] T019 [P] Contract test PUT /api/admin/users/{id} in `backend/tests/contract/test_admin_users_put.py` (qa)
- [ ] T020 [P] Contract test DELETE /api/admin/users/{id} in `backend/tests/contract/test_admin_users_delete.py` (qa)
- [ ] T021 [P] Contract test POST /api/admin/users/{id}/role in `backend/tests/contract/test_admin_users_role_post.py` (qa)
...
- [ ] T026 [P] Frontend E2E: auth and profile flows in `frontend/tests/e2e/auth.spec.ts` (qa)

## Phase 3.3: Core Implementation (Backend)
- [ ] T027 [P] Create DB session and config in `backend/src/db/session.py` (db)
...
- [ ] T032 [P] Implement MFAService (TOTP, backup codes) in `backend/src/services/mfa_service.py` (backend)
- [ ] T033 Implement RBACService policy checks in `backend/src/services/rbac_service.py` (backend)
